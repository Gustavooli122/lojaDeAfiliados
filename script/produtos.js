...
produtos.forEach(produto =>{
    ...
    <div class="flex flex-col lg:justify-between md:justify-between sm:justify-between lg:flex-row md:flex-row sm:flex-row items-center mx-2 mt-2">
      <ul id = "rating" class="flex gap-1">
    ${gerarEstrelas(produto.rating)}
      </ul>
      <p class="text-gray-400  lg:text-xs md:text-xs sm:text-xs text-sm mt-1">R$ ${produto.preco.toFixed(2)}</p>
    </div>
    <div class="mx-2 mt-2">
...
function gerarEstrelas(rating) {
  ...
  for (let i = 1; i <= 5; i++) {
    if (i <= rating) {
      estrelas += `<i class="fa fa-star text-yellow-500  lg:text-xs md:text-xs sm:text-xs text-sm"></i>`;
    } else {
      estrelas += `<i class="fa fa-star text-gray-300 lg:text-xs md:text-xs sm:text-xs text-sm"></i>`;
    }
  }
  return estrelas;
}
